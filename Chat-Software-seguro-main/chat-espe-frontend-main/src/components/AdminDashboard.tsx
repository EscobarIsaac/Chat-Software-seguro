...
import { buildAuthHeaders, getTokenClaims } from '../services/biometricToken';
...
const AdminDashboard: React.FC = () => {
  ...
  const fetchDashboard = async () => {
    ...
    try {
      ...
      if (!headers.Authorization) {
        setError('Falta biometricToken. Inicia sesión en el gateway.');
        setLoading(false);
        return;
      }
  ...
  useEffect(() => {
    ...
    if (claims?.username) {
      setAdminUsername((claims.username || '').trim() || null);
    }
  }, []);
  ...
  const handleDelete = async (roomId: string) => {
    ...
    try {
      const headers = buildAuthHeaders();
      if (!headers.Authorization) {
        alert('Falta biometricToken. Ve al gateway biométrico.');
        return;
      }
      const res = await axios.delete(`${API_BASE_URL}/api/admin/rooms/${roomId}`, {
        withCredentials: true,
        headers,
      });
  ...
  const handleEnter = (room: RoomInfo) => {
    if (!adminUsername) {
      alert('No se pudo determinar el usuario administrador para entrar a la sala.');
      return;
    }
    const pin = (room as any).pin_display || '';
    if (!pin) {
      alert('PIN no disponible para esta sala.');
      return;
    }
    navigate(`/chat/${room.id}`, {
      state: { pin, nickname: adminUsername }
    });
  };
