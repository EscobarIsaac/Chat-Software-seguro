...
from rooms import (create_room, verify_pin, find_room_by_identifier,
                   ensure_normalized_names)
...
# Parchear antes de importar librerías que dependen de sockets/ssl
...
BIOMETRIC_GATEWAY_URL = os.environ.get('BIOMETRIC_GATEWAY_URL',
                                       'http://localhost:7000').rstrip('/')
BIOMETRIC_VALIDATE_PATH = '/api/session/validate'
CHAT_FRONTEND_URL = os.environ.get('CHAT_FRONTEND_URL',
                                   'http://127.0.0.1:5173').rstrip('/')


def _sanitize_origin(candidate: Optional[str]) -> str:
    if not candidate:
        return ''
...
def _extract_biometric_token():
    """Obtiene el token biométrico desde headers, query o cuerpo JSON."""
    ...
    fallback = (
        ...
        or request.args.get('biometricToken')
        or request.cookies.get('biometricToken')
    )
    ...
    if request.is_json:
        ...
        token = payload.get('biometricToken')
        if token:
            return token.strip()
...
def validate_biometric_token(token):
    if not token:
        return None

    try:
        resp = requests.post(
            f"{BIOMETRIC_GATEWAY_URL}{BIOMETRIC_VALIDATE_PATH}",
            json={"token": token},
            timeout=4)
    except requests.RequestException as exc:
        logger.warning("Gateway biométrico inaccesible: %s", exc)
        return None
...
@app.route('/api/biometric/whoami', methods=['GET'])
def biometric_whoami():
    """Devuelve los claims actuales del biometricToken si es válido.

    Se usa desde el frontend para que el botón "Validar token biométrico"
    confirme contra el gateway si hay un token activo y qué rol/usuario tiene.
    """
    token = _extract_biometric_token()
    claims = validate_biometric_token(token)
    if not claims:
        return jsonify({"active": False, "error": "Token inválido o ausente"}), 401
    return jsonify({"active": True, "claims": claims})


@app.route('/biometric/verify', methods=['GET'])
def biometric_verification_redirect():
    ...
    role = (request.args.get('role') or
            ('admin' if flow == 'login' else 'cliente')).strip()
    display_name = (request.args.get('display') or username).strip()

    parent_origin = (_sanitize_origin(request.args.get('origin'))
                     or _sanitize_origin(request.headers.get('Origin'))
                     or _sanitize_origin(request.referrer)
                     or CHAT_FRONTEND_URL)
    ...
    target = f"{BIOMETRIC_GATEWAY_URL}?{urlencode(params)}"
    return redirect(target, code=302)


def biometric_required(role=None):
    def decorator(func):

        @wraps(func)
        def wrapper(*args, **kwargs):
            claims = getattr(request, 'biometric_claims', None)
            token = _extract_biometric_token() if claims is None else None
            ...
            if not claims:
                return jsonify({
                    "error": "Se requiere un biometricToken válido"
                }), 401

            if role and claims.get('role') != role:
                return jsonify({"error": "Permisos insuficientes"}), 403

            request.biometric_claims = claims
            session['admin'] = claims.get('role') == 'admin'
            session['biometric_claims'] = claims
...
@biometric_required(role='admin')
def admin_login():
    claims = getattr(request, 'biometric_claims', {})
    payload = request.get_json(silent=True) or {}
    ...
    token_username = (claims.get('username') or '').strip()
    if username != token_username:
        return jsonify({
            "error": "El token biométrico no corresponde al usuario ingresado."
        }), 403

    session['admin'] = True
    session['admin_username'] = token_username
    return jsonify({
        "success": True,
        "user": token_username,
...
def handle_join(data):
    room_identifier = data.get('room_id') or data.get('room')
    pin = data['pin']
    nickname = data['nickname']
    ip = request.remote_addr
    sid = request.sid

    token = data.get('biometricToken') or _extract_biometric_token()
    claims = validate_biometric_token(token)
    if not claims:
        emit('error', {'msg': 'Biometric token inválido o ausente'})
        return

    if not room_identifier:
        emit('error', {'msg': 'Debes indicar el ID o nombre de la sala'})
        return

    room = find_room_by_identifier(room_identifier)
    if not room:
        emit('error', {'msg': 'Sala no existe'})
        return

    if not verify_pin(room, pin):
