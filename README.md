...
### Integración con el Gateway Biométrico

- Antes de llamar a cualquier endpoint de administración o unirse a una sala, el frontend debe adjuntar el header `X-Biometric-Token` (o el campo `biometricToken` en el body/socket payload) obtenido desde `Biometrico-Software-seguro/gateway`.
- El backend valida el token contra `BIOMETRIC_GATEWAY_URL` (`/api/session/validate`) y rechaza peticiones sin un token válido o con roles incorrectos.
- Configura las siguientes variables en `.env` del backend:
  ```env
  BIOMETRIC_GATEWAY_URL=http://localhost:7000
  BIOMETRIC_GATEWAY_TIMEOUT=3
  BIOMETRIC_TOKEN_HEADER=X-Biometric-Token
  ```
- El pipeline CI (workflow `biometric-ai-scan.yml`) requiere los secretos `GATEWAY_CI_URL` y `GATEWAY_CI_TOKEN` para desencadenar el análisis del motor IA mediante el endpoint `/api/ci/scan`.
